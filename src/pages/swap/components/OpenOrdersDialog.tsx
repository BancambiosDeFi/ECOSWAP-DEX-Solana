      ...
      <Table className={styles.table} aria-label="simple table">
        ...
        <TableBody>
          {openOrdersEntries.map(([market, oos]) => {
            return <OpenOrdersRow key={market.toString()} market={market} openOrders={oos} />;
          })}
        </TableBody>
      </Table>
    </TableContainer>
...
function OpenOrdersRow({
  ...
  const quoteTicker = marketClient
    ...
    : '-';
  const marketName =
    baseTicker && quoteTicker ? `${baseTicker} / ${quoteTicker}` : market.toString();
  const settleDisabled =
    ooAccount.baseTokenFree.toNumber() + ooAccount.quoteTokenFree.toNumber() === 0;
  const closeDisabled =
    ooAccount.baseTokenTotal.toNumber() + ooAccount.quoteTokenTotal.toNumber() !== 0;
  ...
  const settleFunds = async () => {
    ...
    const { transaction, signers } = await marketClient.makeSettleFundsTransaction(
    ...
    <TableRow key={market.toString()}>
      ...
      <TableCell align="center">
        <Select
          ...
          onChange={e =>
            setOoAccount(openOrders.filter(oo => oo.address.toString() === e.target.value)[0])
          }
        >
          {openOrders.map(oo => {
            return (
              <MenuItem key={oo.address.toString()} value={oo.address.toString()}>
                {oo.address.toString()}
              </MenuItem>
            );
          })}
      ...
      <TableCell align="center">{toDisplay(base, ooAccount.baseTokenFree)}</TableCell>
      <TableCell align="center">
        {toDisplay(quote, ooAccount.quoteTokenTotal.sub(ooAccount.quoteTokenFree))}
      </TableCell>
      <TableCell align="center">{toDisplay(quote, ooAccount.quoteTokenFree)}</TableCell>
      <TableCell align="center">
      ...
      <TableCell align="center">
        <Button disabled={closeDisabled} onClick={_closeOpenOrders} className={styles.closeAccount}>
          Close
        </Button>
      </TableCell>
    </TableRow>
